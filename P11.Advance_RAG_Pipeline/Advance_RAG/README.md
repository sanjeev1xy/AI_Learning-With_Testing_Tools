...
> The original spec called for OpenAI for rewriting/generation; this build uses **Groq**
> (`openai/gpt-oss-120b`) instead, since that's the only LLM API key configured in this repo.
...
```bash
cd P11.Advance_RAG_Pipeline/Advance_RAG
python -m venv .venv
