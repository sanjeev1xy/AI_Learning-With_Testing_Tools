...
Create the 5,000 test cases in the /testcase folder for the Advance RAG as a CSV file with jira format. 
...
with techniques that matter at scale on a real corpus (5,000 test cases):
...
- **Generation** — openAI `text-embedding-3-large` (same as Basic)
...
Stage 2 (Chat):
  Question -> rewrite (openAI) -> embed -> dense + sparse search ->
  RRF fuse -> bge-reranker-v2-m3 -> openAI -> grounded answer
